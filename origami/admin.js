...
const ui = {
  ...
  replaceImageInput:    document.getElementById('replace-image-file-input'),
};
...
function countObjectDiffs(a, b) {
  ...
  if (aIsArray || bIsArray) {
    ...
    for (let i = 0; i < minLen; i += 1) total += countObjectDiffs(a[i], b[i]);
  ...
  if (aIsObject || bIsObject) {
    ...
    for (const key of keys) {
      if (!(key in a) || !(key in b)) total += 1;
      else total += countObjectDiffs(a[key], b[key]);
    }
    return total;
  }
...
function newPieceTemplate() {
  return { active: true, title: '', year: '', software: [], mainImage: '', description: '', detailImages: [] };
}
...
function renderDetailItems(detailsContainer, piece) {
  ...
  if (!piece.detailImages.length) {
    ...
    empty.textContent = 'No detail images added yet.';
...
function renderPieces() {
  ...
  if (!pieces.length) {
    const empty = document.createElement('p');
    empty.className = 'muted';
    empty.textContent = 'No pieces yet. Click "+ Add piece" to get started.';
  ...
  pieces.forEach((piece, index) => {
    ...
    const descEl        = node.querySelector('.piece-description');
    const mainImageEl   = node.querySelector('.piece-preview');
    const replaceBtn    = node.querySelector('.replace-image-btn');
    ...
    descEl.value          = piece.description;
    mainImageEl.replaceChildren(createMediaPreview(piece.mainImage));
    ...
    descEl.addEventListener('input', () => {
      piece.description = descEl.value;
      updateChangeIndicator();
    });

    replaceBtn.addEventListener('click', () => {
      if (state.uploadBusy) return;
      state.pendingReplacePiece = { piece, previewEl: mainImageEl };
      ui.replaceImageInput.value = '';
      ui.replaceImageInput.click();
    });
    ...
    node.querySelector('.remove-piece').addEventListener('click', () => {
      if (!confirm(`Remove piece ${index + 1}? This cannot be undone until you save.`)) return;
...
async function saveContent() {
  ...
  try {
    const result = await api('/content', {
      ...
      body:    JSON.stringify({ content: state.content, sha: state.sha, message })
    });
...
function sanitizeFilename(name) {
  ...
  const safeBase = base.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9._()-]/g, '');
...
async function optimizeImageFile(file, maxSide, quality) {
  ...
  if (!ctx) throw new Error('Could not create canvas context for optimization.');
...
function getNormalizedUploadOptions({ folder, optimize, maxSide, quality }) {
  ...
  if (!normalizedFolder.startsWith('images/')) throw new Error('Folder must start with images/.');
  return {
...
async function uploadWithOptions({ sourceFile, folder, optimize, maxSide, quality, messagePrefix = 'upload' }) {
  if (!sourceFile) throw new Error('Select a file first.');
  ...
  const response = await fetch(`${API_BASE}/upload`, { method: 'POST', credentials: 'include', body: formData });
  const payload  = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(payload?.error || `Upload failed (${response.status})`);
  return { path: payload.path, optimized };
}

async function uploadMedia(event) {
  event.preventDefault();
  const sourceFile = ui.uploadFile.files?.[0];
  if (!sourceFile) { setUploadStatus('Select a file first.', true); return; }
  ...
  try {
    ...
    setUploadStatus(`Upload complete${result.optimized ? ' (optimized)' : ''}.`);
...
async function addNewPieceFromUpload(sourceFile) {
  const pieces = getActivePiecesArray();
  if (!Array.isArray(pieces)) { setEditorStatus('Cannot add piece to this collection.', true); return; }

  setUploadBusy(true);
  setEditorStatus(`Uploading "${sourceFile.name}"…`);
  try {
    const result   = await uploadWithOptions({ sourceFile, ...getMainUploadOptions(), messagePrefix: 'upload piece' });
    const newPiece = newPieceTemplate();
    newPiece.mainImage = result.path;
    pieces.push(newPiece);
    renderPieces();
    updateChangeIndicator();
    setEditorStatus(`Piece added. Don't forget to save.`);
  } catch (error) {
    setEditorStatus(error.message, true);
  } finally {
    setUploadBusy(false);
...
async function replaceMainImageFromUpload(sourceFile, piece, previewEl) {
  setUploadBusy(true);
  setEditorStatus(`Uploading replacement image…`);
  try {
    const result      = await uploadWithOptions({ sourceFile, ...getMainUploadOptions(), messagePrefix: 'replace image' });
    piece.mainImage   = result.path;
    previewEl.replaceChildren(createMediaPreview(result.path));
    updateChangeIndicator();
    setEditorStatus(`Image replaced. Don't forget to save.`);
  } catch (error) {
    setEditorStatus(error.message, true);
  } finally {
    state.pendingReplacePiece = null;
    setUploadBusy(false);
    ui.replaceImageInput.value = '';
  }
}
...
async function addDetailMediaFromUpload(sourceFile, targetPiece) {
  if (!targetPiece) { setEditorStatus('Could not find target piece.', true); return; }

  setUploadBusy(true);
  setEditorStatus(`Uploading detail image…`);
  try {
    const result = await uploadWithOptions({ sourceFile, ...getMainUploadOptions(), messagePrefix: 'upload detail' });
    targetPiece.detailImages.push({ src: result.path, caption: '' });
    renderPieces();
    updateChangeIndicator();
    setEditorStatus(`Detail image added. Don't forget to save.`);
  } catch (error) {
    setEditorStatus(error.message, true);
  } finally {
...
function bindEvents() {
  ...
  ui.addPieceFileInput.addEventListener('change', () => {
    ...
    if (file) addNewPieceFromUpload(file);
  });

  ui.addDetailFileInput.addEventListener('change', () => {
    const file = ui.addDetailFileInput.files?.[0];
    if (file) addDetailMediaFromUpload(file, state.pendingDetailPiece);
  });

  ui.replaceImageInput.addEventListener('change', () => {
    const file = ui.replaceImageInput.files?.[0];
    if (file && state.pendingReplacePiece) {
      replaceMainImageFromUpload(file, state.pendingReplacePiece.piece, state.pendingReplacePiece.previewEl);
    }
  });
  ...
  ui.copyUploadPathBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      setUploadStatus('Could not copy path. Please copy manually.', true);
    }
  });
}
